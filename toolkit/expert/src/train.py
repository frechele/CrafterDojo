...
def make_train(config):
    ...
    env = DeathPenaltyWrapper(env, death_penalty=config["DEATH_PENALTY"])
    env = LogWrapper(env)
...
def run_ppo(config):
    ...
    if config["USE_WANDB"]:
        wandb.init(
            ...
            + "-PPO_RNN-"
            + large_number_to_str(config["TOTAL_TIMESTEPS"]),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--death_penalty", type=int, default=10)
