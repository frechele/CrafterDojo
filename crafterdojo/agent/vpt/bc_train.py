...
from crafterdojo.data.vpt_dataset import VPTDataset
from crafterdojo.lib.VPT.agent import CrafterRLAgent, load_model_parameters
...
def main(config, env_entry, logdir: str, fabric: Fabric):
    ...
    samples_per_step = config.trunc_t * real_batch_size
    if fabric.is_global_zero:
        logging.info(f"Batch size in samples: {samples_per_step:,}")
    ...
    config.n_samples = max(
        config.n_samples, config.n_epochs * epoch_len * samples_per_step
    )
    ...
    def run_validation_internal():
        ...
        with timer.time("validation"):
            val_loss = run_validation(config, policy, val_dataloader, device, fabric)
        metrics_log.update(
            {
                "val_loss": val_loss,
            }
    ...
    while n_steps * samples_per_step <= frame_limit and not end_training:
        policy.train()
        for obs, actions, firsts, _ in timer.time_iter(train_dataloader, "dataloader"):
            n_batches += 1
            if n_steps * samples_per_step >= frame_limit:
                end_training = True
                break

            B = firsts.shape[0]
            T = firsts.shape[1]
            timer.throughput("samples", T * real_batch_size)

            for t in range(0, T, trunc_t):
                num_samples_processed = n_steps * samples_per_step
                lr = get_lr(config, num_samples_processed)
                    ...
                    metrics_log.update(
                        {
                            "loss": avg_loss,
                            "step": n_steps,
                            "processed_samples": n_steps * samples_per_step,
                            "epoch": n_steps / epoch_len,
                            "learning_rate": lr,
                ...
                if (n_steps * samples_per_step) >= next_snapshot_n_samples:
                    state_dict = policy.state_dict()
                    fabric.save(
