...
class Player(Object):
  ...
  def __init__(self, world, pos, custom_items=None):
    super().__init__(world, pos)
    self.facing = (0, 1)

    base_items = copy.deepcopy(constants.items)
    if custom_items is not None:
      for name, quantity in custom_items.items():
        base_items[name]['initial'] = quantity

    self.inventory = {
        name: info['initial'] for name, info in base_items.items()}
