...
from .types import VPTStates, VPTRolloutBufferSamples


class VPTBuffer(DictRolloutBuffer):
    """
    ...
    def add(self, *args, vpt_states: VPTStates, **kwargs) -> None:
        """
        :param hidden_states
        """
    ...
    def _get_samples(
        ...
        return VPTRolloutBufferSamples(
            ...
            observations={
                key: self.pad(obs[batch_inds]).reshape((padded_batch_size,) + obs.shape[1:])
                for key, obs in self.observations.items()},
