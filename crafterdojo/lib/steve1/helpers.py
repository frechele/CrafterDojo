...
def batch_recursive_objects(ls, check_shape: bool = False):
    ...
    if isinstance(first, dict):
        ...
        return [batch_recursive_objects([lst[i] for lst in ls]) for i in range(len(first))]
    elif isinstance(first, tuple):
        # Similar to the above, but for tuples
        return tuple(batch_recursive_objects([lst[i] for lst in ls]) for i in range(len(first)))
    elif first is None:
        return None
    else:
