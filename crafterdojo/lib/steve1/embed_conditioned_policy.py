...
from crafterdojo.lib.VPT.lib.action_head import make_action_head
from crafterdojo.lib.VPT.lib.impala_cnn import ImpalaCNN
from crafterdojo.lib.VPT.lib.scaled_mse_head import ScaledMSEHead
from crafterdojo.lib.VPT.lib.tree_util import tree_map
from crafterdojo.lib.VPT.lib.util import FanInInitReLULayer, ResidualRecurrentBlocks
from crafterdojo.lib.VPT.lib.misc import transpose
...
class MinecraftPolicy(nn.Module):
    ...
    def __init__(
        ...
        goal_embed_dim=512,  # MODIFIED (added this)
        **unused_kwargs,
    ):
        super().__init__()
        ...
        # MODIFIED (added this)
        self.mineclip_embed_linear = th.nn.Linear(goal_embed_dim, hidsize)
    ...
    def forward(self, ob, state_in, context):
        ...
        # MODIFIED (added this)
        goal_embed = ob["goal_embed"].reshape(b * t, -1)
        # Normalize goal_embed (doesn't work because the norm is way too small then?)
        # goal_embed = F.normalize(goal_embed, dim=-1)
        goal_embed = self.mineclip_embed_linear(goal_embed)
        goal_embed = goal_embed.reshape(b, t, -1)
        # x = x + goal_embed
...
class MinecraftAgentPolicy(nn.Module):
    ...
    def inject_lora(self, lora_alpha, lora_r):
        lora_config = LoraConfig(
            lora_alpha=lora_alpha,
            r=lora_r,
            bias="none",
            target_modules=["q_layer", "k_layer", "v_layer", "r_layer"],
        )
        self.net.recurrent_layer = inject_adapter_in_model(lora_config, self.net.recurrent_layer)
    ...
    def act(self, obs, first, state_in, stochastic: bool = True,
        ...
        if cond_scale is not None and cond_scale != 0:
            bsz = obs["img"].shape[0]
            ...
            obs["goal_embed"][1] = th.zeros_like(obs["goal_embed"][1])
        ...
        if cond_scale is not None and cond_scale != 0:
            ...
            def dynamic_cond_scale(conditional: th.Tensor, unconditional: th.Tensor, cond_scale):
                # conditional, unconditional = conditional.exp(), unconditional.exp()

                # sim = th.nn.functional.cosine_similarity(unconditional, conditional, dim=-1)
                # sim = th.nn.functional.kl_div(unconditional, conditional, reduction='none', log_target=True).sum(dim=-1)
                # if sim > 0.1:
                #     cond_scale = cond_scale * 2

                result = (((1 + cond_scale) * conditional) - (cond_scale * unconditional)).unsqueeze(0)
                result = result - th.logsumexp(result, dim=-1, keepdim=True)

                # uncond_act = unconditional.argmax(dim=-1).item()
                # cond_act = conditional.argmax(dim=-1).item()
                # result_act = result.argmax(dim=-1).item()

                # if uncond_act == cond_act and cond_act == result_act:
                #     print(f"similarity: {sim.item():.5f}",
                #         f"cond_scale: {cond_scale:.2f}",
                #         f"unconditional: {uncond_act} ({unconditional.exp().max():.2f})",
                #         f"conditional: {cond_act} ({conditional.exp().max():.2f})",
                #         f"result: {result_act} ({result.exp().max():.2f})",
                #         sep=", ")
                # else:
                #     print(f"similarity: {sim.item():.5f}",
                #         f"cond_scale: {cond_scale:.2f}",
                #         f"unconditional: {uncond_act} ({unconditional.exp().max():.2f})",
                #         f"conditional: {cond_act} ({conditional.exp().max():.2f})",
                #         f"result: [bold red]{result_act}[/bold red] ({result.exp().max():.2f})",
                #         sep=", ")

                return result

            pd = tree_map(lambda x: dynamic_cond_scale(x[0], x[1], cond_scale), pd)
